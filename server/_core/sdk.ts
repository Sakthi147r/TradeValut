...
class OAuthService {
  constructor(private client: ReturnType<typeof axios.create>) {
    if (ENV.oAuthServerUrl) {
      ...
      console.log("[OAuth] Note: OAUTH_SERVER_URL is not set (external OAuth login disabled).");
    }
  }
...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    if (Date.now() - lastSigned > FIFTEEN_MINUTES_MS) {
      db.upsertUser({
        openId: user.openId,
        lastSignedIn: signedInAt,
      }).catch(err => console.error("[Auth] Background lastSignedIn update failed:", err));
    }
